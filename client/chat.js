  ...
  const st = document.createElement('style'); st.textContent = CHAT_CSS; document.head.appendChild(st);
    ...
    function chatDecorate() {                       // idempotent: bubble class from the kind tag + md
      chatRowsEl.querySelectorAll('li').forEach(li => {
        ...
        const v = li.querySelector('.v'); if (!v) { if (li.className !== cls) li.className = cls; return; }
        if (v.dataset.raw === undefined) v.dataset.raw = v.textContent;
        // A think row is "<status>\n<one line per finished call>".  Split it so the status keeps
        // its own wrapping and the list can be hidden as a block.
        if (cls === 'think') {
          const raw = v.dataset.raw, nl = raw.indexOf('\n');
          const head = nl < 0 ? raw : raw.slice(0, nl);
          const rest = nl < 0 ? '' : raw.slice(nl + 1).replace(/\n+$/, '');
          const html = chatMd(head) + (rest ? '<span class="dt">' + chatMd(rest) + '</span>' : '');
          if (v.innerHTML !== html) v.innerHTML = html;
          // className is rebuilt from scratch every pass, so the open flag lives in the dataset
          // where a repaint cannot lose it -- rows update several times a second while a tool runs.
          const want = 'think' + (rest ? ' more' : '') + (rest && li.dataset.open === '1' ? ' open' : '');
    ...
    // Delegated, because rows are replaced wholesale by the warp client and a handler bound to a
    // row would go with it.  Only rows that actually have a detail react.
    chatRowsEl.addEventListener('click', e => {
      const li = e.target.closest && e.target.closest('li.think.more');
      if (!li) return;
